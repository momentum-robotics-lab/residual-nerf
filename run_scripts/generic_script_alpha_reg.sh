...
--results_dir $OUT_FOLDER --ckpt scratch  --wandb --wandb_name $WORKSPACE'_normal_alpha_reg' --wandb_project $WORKSPACE
...
--ckpt scratch --wandb --wandb_name $WORKSPACE'_bg_alpha_reg' --wandb_project $WORKSPACE --results_dir $OUT_FOLDER

# # run the residual nerf

python3 main_nerf.py $DATA_PATH --workspace $WORKSPACE'_res' --bg_ckpt $BG_CHECKPOINT -O --dt_gamma 0  --iters $ITERATIONS --type wrap --d_thresh 3.0 --downscale $DOWNSCALE --min_near 0.2 --mixnet_reg 0.001 --eval_interval 10 \
--results_dir $OUT_FOLDER --ckpt scratch --wandb --wandb_name $WORKSPACE'_res_alpha_reg' --wandb_project $WORKSPACE