...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4,D_thresh=10.0, bg_model=None,nears_force=None,fars_force=None,**kwargs):
        ...
        if self.training:
            ...
            if len(sigmas.shape) == 2:
                ...
                images = []
                for k in range(K):
                    weights_sum, depth, image, dex_depth = raymarching.composite_rays_train(sigmas[k], rgbs[k], deltas, rays, T_thresh,D_thresh=D_thresh,return_dex=True,bg_sigmas=bg_sigmas[k])
                    image = image + (1 - weights_sum).unsqueeze(-1) * bg_color
        ...
        results['nerf_depth_raw'] = depth_raw
        results['rgbs'] = rgbs
    ...
    def render(self, rays_o, rays_d, staged=False, max_ray_batch=4096,bg_model=None,d_thresh_force=None,D_thresh=10.0, **kwargs):
